...
import { answerList, mapList, questionList } from './map-list';
...
export default defineComponent(() => {
    ...
    const addAnswer = (item: string) => {
        ...
        ansList = [];
        loadingText.value = '稍等...';
        loadingTimer = setTimeout(() => {
            loadingText.value = '对方正输入中...';
        }, getLoadingDelayTime());
        timer = setTimeout(() => {
        ...
        <div class="chat-wrapper" style={{marginBottom: chatBottomHeight.value + 50 + 'px' }}>
            {
                msgList.map(item => (
                    <li class={['flex pd10 align-start', item.direction === 'left' ? 'justify-start' : 'justify-end']}>
                        ...
                        <div class={['mg-110 bubble', item.direction === 'left' ? 'bubble-left' : 'bubble-right']}>
                            <p class="bgff 1h20 fs12" v-html={item.message}></p>
                        </div>
    ...
    const answerDom = () => ansList.length > 0 ? (
        ...
        <div class="answers-wrapper max640">
            <div class="loadingtxt fs12 1h20 mg10 text-center"
    ...
    return () => (
        <div class="pd-nav">
            <NavBar fixed title="聊天室" left-arrow onClick-left={onClickLeft}></NavBar>
            { messageDom() }
            { answerDom() } 
        </div>
    )
